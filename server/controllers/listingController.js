...
// controller to adding listing from database
...
export const markedFeature= async (req, res)=>{
    try {
        const {id} = req.params; 
        const {userId} = await req.auth();
         if(req.plan !== "premium"){
         ...
         //unset all other feature listings
         await prisma.listing.updateMany({
            where : {ownerId :  userId},
            data : {featured : false}
         })

         //marked the listing as feaure
         await prisma.listing.update({
            where : {id},
            data : {featured : true}
         })
...
export const getAllUserOrders = async (req,res)=>{
    ...
    const {userId} = await req.auth();
       ...
       // attack the credential to each order
...
export const purchaseAccount = async (req, res) => {
  try {
      ...
      line_items: [
        {
          price_data: {
            currency: "usd",
            product_data: {
              name: `Purchase Account @${listing.username} of ${listing.platform}`
            },
    ...
    return res.json({ paymentLink: session.url, transaction });

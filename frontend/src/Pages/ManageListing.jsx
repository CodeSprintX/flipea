...
const ManageListing = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    // TODO: Implement form submission logic (e.g., API call to create/update listing)
  }
      ...
      <form onSubmit={handleSubmit} className='space-y-8'>
        ...
        <Section title='Account Metrics'>
          <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'>
            ...
            <InputField label='Monthly View/Impressions' type='number' min={0} value={formData.monthly_views} placeholder='100000'  onChange={(v)=>handleInputChange('monthly_views', v)}  />
        ...
        <Section title='Pricing & Description'>
          ...
          <TextareatField label='Description *' value={formData.description} onChange={(v)=>handleInputChange('description', v)} required={true}/>
        </Section>

        {/* images */}

        <Section title='Screeshots & Proof'>
        ...
        <div className='flex justify-end gap-3 text-sm'>
          ...
          <button type='submit' className='px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors' >{isEditing ?'Update Listing' :'CreateListing'}</button>
        </div>
      </form>
      </div>
...
const SelectField = ({ label, options, value, onChange, required = false}) =>(
  <div>
    <label className='block text-sm font-medium text-gray-700 mb-2'>{label}</label>
    <select value={value} onChange={(e)=> onChange(e.target.value)} className='w-full px-3 py-1.5 text-gray-600 border rounded-md focus:outline-none focus:ring-2 foucus:ring-indigo-500 border-gray-300 ' required={required}>
...
const TextareatField = ({label, value, onChange, required = false}) =>(
  <div>
    <label className='block text-sm font-medium text-gray-700 mb-2'>{label}</label>

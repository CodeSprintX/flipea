import { ArrowDownCircleIcon, BanIcon, CheckCircle, Clock, CoinsIcon, DollarSign, Edit, Eye, EyeIcon, EyeOff, EyeOffIcon, LockIcon, Plus, StarIcon, TrashIcon, TrendingUp, Users, WalletIcon, XCircle } from 'lucide-react'
import  { useState } from 'react'
import { useDispatch, useSelector } from 'react-redux'
import {useNavigate} from 'react-router-dom'
import StatCard from '../components/StatCard'
import { platformIcons } from '../assets/assets'
import CreditentialSubmission from '../components/CreditentialSubmission'
import WithdrawLModel from '../components/WithdrawLModel'
...
const MyListings = () => {
  ...
  const [showCreditialSubmission, setShowCreditialSubmission] = useState(null)
  ...
  <div className="invisible group-hover:visible absolute right-0 top-0 pt-4.5 z-10">
    <div className="bg-white text-gray-600 text-xs rounded border border-gray-200 p-2 px-3">
      {!listing.isCredentialSubmitted && (
        <>
          <button onClick={()=>setShowCreditialSubmission(listing)} className="flex items-center gap-2 text-nowrap">Add Credentials</button>
  ...
  <StarIcon onClick={()=>markedAsFeatured(listing.id)}
    size={18}
  ...
  <p className='text-sm text-gray-600'><span>@{listing.username}</span></p>
    ...
    <div className='space-y-4'>
      ...
      <div className='flex items-center justify-between pt-3 border-t border-gray-200'>
        ...
        <div className='flex items-center space-x-2'>
          {listing.status !== "sold" && (
            <button onClick={()=>deleteListing(listing.id)} className='p-2 border border-gray-300 rounded-lg hover:bg-hover-50 hover:text-red-500'>
          ...
          <button onClick={()=>toggleStatus(listing.id)} className='p-2 border border-gray-300 rounded-lg hover:bg-hover-50 hover:text-purple-600'>
...
{showCreditialSubmission && (
  <CreditentialSubmission listing={showCreditialSubmission} onClose={()=>setShowCreditialSubmission(null)} />
)}
{showWithdrawl && (
    <WithdrawLModel onClose={()=>setShowWithdrawl(null)}/>
)}
 {/* footer */}
      <div className='bg-white border-t border-gray-200 p-4 text-center mt-28'>
       <p className='text-sm text-gray-500'>
         © 2025 <span className='text-indigo-600'>FlipEarn</span>. All Right reserved.
       </p>
      </div>

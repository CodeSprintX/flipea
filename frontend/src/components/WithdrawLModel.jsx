...
const WithdrawLModel = ({onClose}) => {
  ...
  return (
    <div className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-100 flex flex items-center justify-center sm:p-4'>
        <div className='bg-white sm:rounded-lg shadow-2xl w-full max-w-lg h-screen sm:h-auto flex flex-col'>
            ...
            <form onSubmit={handleSubmission} className='flex flex-col items-start gap-4 p-4 overflow-y-scroll'>
                ...
                {/* submit */}
                <button type='submit' className='bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 my-4 rounded-md'>Apply for Withdrawl</button>
            </form>
        </div>
